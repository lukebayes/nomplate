...
class Nomtml extends Nomplate {
  static initClass() {
    ...
    this.htmlFiveNodes.forEach(node => {
      return this.prototype[node] = function(...args) {
        args.unshift(node);
        if (this.node === undefined) {
          throw `Nomtml unable to find node: ${node}`;
        }
        return this.node.apply(this, args);
      };
    }
    );
  }
  ...
  inLineScript(srcOrBlock) {
    return srcOrBlock instanceof Function ||
  ...
  javascript(srcOrBlock) {
    if (this.inLineScript(srcOrBlock)) {
      ...
      return this.node('script', {src: srcOrBlock, type: 'text/javascript'});
    }
  }

  // Write a Standard Stylesheet tag:
  stylesheet(href, type = 'text/css') {
    return this.node('link', {rel: 'stylesheet', type, href});
  }
  ...
  image(src, title, alt) {
    if (!title) { title = src; }
    if (!alt) { alt = title; }
    return this.node('img', {src, title, alt});
  }

  anchor(href, text, title = null) {
    if (!title) { title = text; }
    return this.node('a', { href, title }, text);
  }
}
Nomtml.initClass();

export default Nomtml;
